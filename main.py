...
def convert_video_to_mp3(input_file, output_path):
    ...
    output_file = os.path.join(output_path, "output_audio.mp3")
...
def download_multiple_video():
    saving_directory = directory_sanitization("enter where you want the videos downloaded")
    url_to_download = []
    adding_link = True
    while adding_link:
        url = youtube_url_sanitization('Enter the youtube URL')
    ...
    for e in url_to_download:
        command = ['python', 'main.py', '-u', e, '-o', saving_directory, '-mp34', 'mp4']
        subprocess.run(command)

def download_multiple_music():
    saving_directory = directory_sanitization("enter where you want the musics downloaded")
    url = youtube_url_sanitization('Enter the youtube URL')
...
def main_menue():
    ...
    menu[user_menu_sanitisation(f"enter a number in between 1 and {menu_maximum_number}", f"{menu_maximum_number}")][1]()
